...
// Função para desenhar um caractere 2x2
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
    ...
    if (c >= '0' && c <= '9') {
        ...
        switch (c) {
            ...
            case ':': index = 68 * 8; break;
            case '>': index = 69 * 8; break;
            default: return;
        }
    }
            ...
            if (line & (1 << j)) {
                for (uint8_t dx = 0; dx < 2; ++dx) {
                    for (uint8_t dy = 0; dy < 2; ++dy) {
                        ssd1306_pixel(ssd, x + i * 2 + dx, y + j * 2 + dy, 1);
                    }
                }
            }
        }
...
void ssd1306_draw_string(ssd1306_t *ssd, const char *str, uint8_t x, uint8_t y)
{
    while (*str) {
        if (*str == '\n') {
            // Se encontrar um caractere de nova linha, mova para a próxima linha
            x = 0;
            y += 8 * 2 - 6; // Ajuste o incremento de y para reduzir ainda mais o espaçamento entre linhas
        } else {
            ssd1306_draw_char(ssd, *str, x, y);
            x += 8 * 2; // Ajuste o incremento de x para o novo tamanho do caractere

            // Se a próxima posição x ultrapassar a largura do display, mova para a próxima linha
            if (x + 8 * 2 > ssd->width) {
                x = 0;
                y += 8 * 2 - 6; // Ajuste o incremento de y para reduzir ainda mais o espaçamento entre linhas
            }
        ...
        // Se a próxima posição y ultrapassar a altura do display, pare de desenhar
        if (y + 8 * 2 > ssd->height) {
            break;
        }
